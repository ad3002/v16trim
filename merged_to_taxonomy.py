...
if __name__ == "__main__":
        ...
        for i, (header, seq) in enumerate(sc_iter_fasta_brute(silva_align)):
            ...
            if i % 25000 == 0:
                print(
                    sample,
                    "BUILDDB",
                    f"total: {i}, uncalled: {len(uncalled)} {round((time.time()-start_time)/60,2)} min",
                )
            if not "Bacteria" in header:
                continue
            poses, hd = find_primer(
                seq, left_pattern, start=left_pos - 100, end=left_pos + 100
            )
            if hd > 3:
            ...
            for i in range(6):
                ...
                if len(taxon) == 1:
                    ...
                    taxon = [
                        x
                        for x in taxon
                        if not x.lower() in ["uncultured", "unidentified"]
                    ]
                    if len(taxon) == 1:
                        taxons.append(taxon[0])
                        taxon = list(
    ...
    print(
        f"...load {i} reads and {len(merged_reads)} uniq reads in {round((time.time()-start_time)/60,2)} min"
    )
    result = []
    ...
    print(
        f"Process dada {sample} of size {len(merged_reads)} and {round((time.time()-start_time)/60,2)} min"
    )
        ...
        if len(hits) == 1:
            ...
            for t in range(7):
                ...
                if len(c):
                    ...
                    hit.append("")
        result.append(
            (sample, rid_i, merged_reads[read], read, good_kmer, found_minhd, hit)
        )
        statistics["ok"] += 1
        statistics["hds"][found_minhd] += 1
        statistics["taxons"][tuple(hit)] += 1
